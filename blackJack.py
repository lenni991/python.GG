import random
from replit import clear

def deal():
  cards=[11,2,3,4,5,6,7,8,9,10,10,10,10]
  card=random.choice(cards)
  return card


def calc_scores(cards):
  '''take a list of cards and return the score of calculated from the cards'''
  if sum(cards)==21 and len(cards) ==2:
    return 0    
  if 11 in cards and sum(cards)>21:
     cards.remove(11)
     cards.append(1)

  return sum(cards)


def compare__(user_score, computer_score):
  if user_score==computer_score:
    return 'Draw!'
  elif computer_score==0:
    return 'Lose!'
  elif user_score==0:
    return 'Win with blackJack!'
  elif user_score>21:
    return 'you went over!'
  elif computer_score>21:
    return 'opponent went over!'
  elif user_score>computer_score:
    return 'YOU WIN ⚜️!'
  else:return 'You Lose'
def ___game():
  user_cards=[]
  computer_cards=[]
  over=False

  for _ in range(2):
    user_cards.append(deal())
    computer_cards.append(deal())

  while not over:
    user_score=calc_scores(user_cards)
    computer_score=calc_scores(computer_cards)

    print(f'your cards: {user_cards}, current score:{user_score}')
    print(f'computers cards: {computer_cards[0]}')


    if user_score==0 or computer_score == 0 or user_score>21:
      over=True
    else:
      letsgo=input("Type 'y' to get another card, Type 'n' to pass: ")
      if letsgo=="y":
        user_cards.append(deal())
      else: over=True
      

  while computer_score !=0 and computer_score<17:
    computer_cards.append(deal())
    computer_score=calc_scores(computer_cards)
  

  print(compare__(user_score,computer_score))

while input('Do you want to play again (y/n)?')=="y":
  clear()
<<<<<<< HEAD
  ___game()
=======
  ___game()
>>>>>>> 59b4f105685684cb15e79243a4df4929669367bd
